...
| [ADR-0022: persistenter Session-Host](../adr/0022-session-host-outlives-clients.md) | teilweise ersetzt durch ADR-0023 | Client-unabhängige Lebensdauer und fachliche Session-Regeln bleiben; Ein-Session-pro-Server und MCP-Präferenz entfallen. |
| [ADR-0023: lokaler Multi-Session-Server, CLI zuerst](../adr/0023-local-server-manages-multiple-sessions.md) | angenommen | Ein Server verwaltet mehrere unabhängige Sessions. Session-Verwaltung und Spiel-Commands bleiben getrennt; CLI-Bedienung ohne MCP und ohne zusätzliche UIs. |
| [ADR-0024: Server, Client und CLI trennen](../adr/0024-separate-server-client-and-cli.md) | angenommen | `server`, `client` und `cli` ersetzen das Sammelmodul `host`; Agent-Platzierung, Exports, Features und Crates bleiben gesondert zu entscheiden. |
