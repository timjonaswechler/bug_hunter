...
let tiny = NSRect(x: desktop.maxX - 6, y: desktop.maxY - 6, width: 4, height: 4)
...
func acknowledge(_ state: String) {
    ...
    print("{\"deck_state\":\"\(state)\",\"pid\":\(ProcessInfo.processInfo.processIdentifier)," +
          "\"key\":\(deck.isKeyWindow),\"x\":\(frame.origin.x),\"y\":\(frame.origin.y)," +
